...
updater.addon = "layout_companion"
...
def get_user_preferences(context=None):
    ...
    addon_name = "LayoutCompanion" 

    prefs_container = getattr(context, "preferences", None)
    if not prefs_container:
        return None

    addon_entry = prefs_container.addons.get(addon_name)
    if not addon_entry:
        return None
...
def register(bl_info):
    ...
    updater.user = "Arciwise2000"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "layout_companion"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/Arciwise2000/layout_companion"
